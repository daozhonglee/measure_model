...
debug_mode = 2
# 支持库：onnxruntime-gpu
# model_path = "r'/Users/shanquan/code/opencv_code/measure/v2/upload_files/runs/train-data-add-3-1280-s/exp3/weights/best.onnx'"
device = torch.device('mps')
model_path = "./runs/train-data-add-3-1280-s/exp3/weights/best.pt"
model = YOLO(model_path).to(device=device)
# model = onnx.load(model_path)  # pretrained YOLOv8n model
# sess_options = ort.SessionOptions()
# sess = ort.InferenceSession(model_path, sess_options)
# sess.set_providers(['CoreMLExecutionProvider'])
# inpput_name = sess.get_inputs()[0].name
...
if __name__ == '__main__':
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            # 进球次数
            cv2.putText(frame, f'Score: {score_count}', (10, 300), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 0), 2)
            # 发球次数
            cv2.putText(frame, f'Shot_: {shot_count}', (10, 250), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 0), 2)
            # 当前第几帧
            cv2.putText(frame, f'Frame: {frame_count}', (10, 400), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 0), 2)
            # 计算命中率

            if shot_count > 0:
                hit_rate = (score_count / shot_count) * 100
                cv2.putText(frame, f'Hit rate: {hit_rate:.2f}%', (10, 350), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 0), 2)
